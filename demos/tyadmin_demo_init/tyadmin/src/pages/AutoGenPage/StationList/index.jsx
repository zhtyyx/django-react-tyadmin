...
import { addStation, queryStation, removeStation, updateStation, queryStationVerboseName, queryStationListDisplay, queryStationDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryRegion, queryRegionVerboseName} from '@/pages/AutoGenPage/RegionList/service';import {queryCity, queryCityVerboseName} from '@/pages/AutoGenPage/CityList/service';import {queryUserProfile, queryUserProfileVerboseName} from '@/pages/AutoGenPage/UserProfileList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateStation(value, current_id);
      hide();
        ...
        hideInSearch: true,
                      ...
                      message: 'region为必填项',
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, regionForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, regionVerboseNameMap);
                            },
  ...
  useEffect(() => {
    queryStationDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryStationListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                const [regionForeignKeyList, setRegionForeignKeyList] = useState([]);
                                useEffect(() => {
                                queryRegion({all: 1}).then(value => {
                                     setRegionForeignKeyList(value);
                                });
                                }, []);
                                const [regionVerboseNameMap, setRegionVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryRegionVerboseName().then(value => {
                                    setRegionVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryStation, table_columns, 'station-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索station" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'station-select')
                    }
                  }}
                  selectedKeys={[]}
                >
